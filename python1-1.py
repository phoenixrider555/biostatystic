...
# Применяем функцию и сохраняем результат
result = df.apply(lambda row: chi2_component(row, None), axis=1)

# Разбиваем на две колонки
df["χ²"] = result.apply(lambda x: x[0])
df["p"] = result.apply(lambda x: x[1])
